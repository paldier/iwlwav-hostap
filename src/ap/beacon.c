...
#define MAX_PROBERESP_LEN 768
	...
	*pos++ = hapd->conf->ssid.ssid_len;
	...
	if ((hapd->conf->vendor_vht || hapd->iconf->ieee80211ac) &&
		!hapd->conf->disable_11ac) { /* IOP with STA transmitting directly VHT IE */
		      ...
		      int ssi_signal, int snr_db)
{
	...
	if(hapd->iconf->multibss_enable) {
		resp = hostapd_gen_probe_resp(hapd->iface->bss[MULTIBSS_REFERENCE_BSS_IDX],
					      mgmt, elems.p2p != NULL,
	...
	if(hapd->iconf->multibss_enable) {
		ret = hostapd_drv_send_mlme_csa(hapd->iface->bss[MULTIBSS_REFERENCE_BSS_IDX], resp, resp_len, noack,

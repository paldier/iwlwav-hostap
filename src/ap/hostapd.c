...
#define HE_OPER_IN_KERNEL_MIN_SIZE 6
#define HE_OPER_VHT_OPER_INFO_SIZE 3
	u8 he_operation_hw[sizeof(hapd->iconf->he_oper)];
	u8 he_oper_bss_color_disable_flag = (hapd->iface->conf->he_oper.bss_color_info & HE_OPERATION_BSS_COLOR_DISABLED);
	if(hostapd_drv_get_he_operation(hapd, (void*)&he_operation_hw) < 0) {
		 wpa_printf(MSG_ERROR, "Failed to get HE Operation from driver");
	} else {
		/* First part of he_oper (hostapd defined) and he_operation_hw (kernel defined) are identical,
		 * the rest is different and will be set below. */
		os_memcpy((void*)&hapd->iconf->he_oper, (void*)&he_operation_hw,
							 MIN(HE_OPER_IN_KERNEL_MIN_SIZE, sizeof(hapd->iconf->he_oper)));

		int co_hosted_elem_offset = HE_OPER_IN_KERNEL_MIN_SIZE;

		/* Check VHT Operation Information Present */
		if(hapd->iface->conf->he_oper.he_oper_params[HE_OPERATION_CAP1_IDX] &
																		HE_OPERATION_CAP1_VHT_OPER_INFO_PRESENT)
																												co_hosted_elem_offset += HE_OPER_VHT_OPER_INFO_SIZE;

		/* Check if Co-Hosted (a.k.a Co-Located) BSS field is present and set its value reported by driver */
		if(hapd->iconf->he_oper.he_oper_params[HE_OPERATION_CAP1_IDX]
																								 & HE_OPERATION_CAP1_CO_LOCATED_BSS) {
				if(co_hosted_elem_offset < sizeof(he_operation_hw))
					hapd->iconf->he_oper.max_co_located_bssid_ind = he_operation_hw[co_hosted_elem_offset];
		}

    if(he_oper_bss_color_disable_flag) {
      he_operation_hw[HE_OPERATION_CAP3_IDX] |= (HE_OPERATION_BSS_COLOR_DISABLED);
    } else {
      he_operation_hw[HE_OPERATION_CAP3_IDX] &= (~HE_OPERATION_BSS_COLOR_DISABLED);
    }

    hapd->iconf->he_oper.bss_color_info = he_operation_hw[HE_OPERATION_CAP3_IDX];

    if (hostapd_drv_set_he_operation(hapd, (void*)&he_operation_hw) < 0)
...
{
	const struct hostapd_data *hapd = timeout_ctx;
...
void send_cred_request_to_ui(struct hostapd_data *hapd)
{
	...
	eloop_register_timeout(time_to_request, 0, terminate_hostapd, 0, hapd);
}
...
int hostapd_setup_bss(struct hostapd_data *hapd, int first)
{
	...
	if (hapd->conf->sFourAddrMode != -1){
		if (hostapd_drv_vendor_cmd(hapd, OUI_LTQ, LTQ_NL80211_VENDOR_SUBCMD_SET_4ADDR_MODE,
				(u8*)&hapd->conf->sFourAddrMode,
				sizeof(hapd->conf->sFourAddrMode), NULL))
			wpa_printf(MSG_WARNING, "set sFourAddrMode failed");
	}
	...
	if (hapd->conf->num_sAddPeerAP > 0 && hapd->conf->sAddPeerAP != NULL)
	{
		...
		os_memcpy(data8, &sa_family, sizeof(uint16_t));

		for (i=0; i < hapd->conf->num_sAddPeerAP; i++)
		{
			os_memcpy(data6, hapd->conf->sAddPeerAP[i], sizeof(macaddr));
...
static void hostapd_post_up_vendor_cmd_mu_type(struct hostapd_data *hapd, int sDynamicMuTypeDownLink, int sDynamicMuTypeUpLink)
{
	struct wpabuf *rsp = NULL;
	int *prevMuConfig = 0;

	if((sDynamicMuTypeDownLink == -1) && (sDynamicMuTypeUpLink == -1))
		return; /*nothing to set*/

	rsp = wpabuf_alloc(DYNAMIC_MU_TYPE_SIZE * sizeof(int) + sizeof(struct wpabuf));
	if(!rsp){
		wpa_printf(MSG_WARNING, "failed to allocate wpabuf");
		return;
	}
	prevMuConfig = (int *)rsp->buf;

	if(hostapd_drv_vendor_cmd(hapd, OUI_LTQ, LTQ_NL80211_VENDOR_SUBCMD_GET_DYNAMIC_MU_TYPE,
		(u8*)prevMuConfig, DYNAMIC_MU_TYPE_SIZE * sizeof(int), rsp)){
		wpa_printf(MSG_WARNING, "get dynamic mu parameter failed");
		wpabuf_free(rsp);
		return;
	}

	if(sDynamicMuTypeDownLink != -1)
		prevMuConfig[0] = sDynamicMuTypeDownLink;
	if(sDynamicMuTypeUpLink != -1)
		prevMuConfig[1] = sDynamicMuTypeUpLink;

	if(hostapd_drv_vendor_cmd(hapd, OUI_LTQ, LTQ_NL80211_VENDOR_SUBCMD_SET_DYNAMIC_MU_TYPE,
		(u8*)prevMuConfig, DYNAMIC_MU_TYPE_SIZE * sizeof(int), NULL))
...
static void hostapd_post_up_vendor_cmd_guard_interval(struct hostapd_data *hapd, int sFixedLtfGi, int ieee80211ax, int ieee80211ac)
{
	...
	switch (sFixedLtfGi){
		...
		case GUARD_INTERVAL_800:
			if(ieee80211ac && !ieee80211ax)
				iwFormat[1] = GI_CP_MODE_MED_CP_SHORT_LTF;
			else if(!ieee80211ac && ieee80211ax)
      ...
      ((channel->flag & HOSTAPD_CHAN_DFS_MASK) != HOSTAPD_CHAN_DFS_AVAILABLE))) {
			...
			for (j = 0; j < (iface->set_freq_done ? iface->num_bss : 1); j++) {
				...
				if (hostapd_set_freq(hapd, hapd->iconf->hw_mode, iface->freq,
						hapd->iconf->channel, hapd->iconf->ieee80211n,
...
int hostapd_reconf_iface(struct hostapd_iface *hapd_iface, int changed_idx)
{
	...
	do {
		...
		if (found_missing_bss) {
			...
			if(start_ctrl_iface_bss(hapd_iface->bss[hapd_iface->num_bss - 1] )){
				wpa_printf(MSG_ERROR, "Failed adding new BSS (%s) to control interface",
						new_conf->bss[hapd_iface->num_bss - 1]->iface);
				hostapd_config_free(new_conf);
				return -1;
			}
		}

				      ...
				      char *buf, size_t buflen)
{
	...
	if ((sta->flags & WLAN_STA_HE) && sta->he_capabilities) {
		u8 *he;

		he = (u8*) &sta->he_capabilities->he_mac_capab_info;
		ret = os_snprintf(buf + len, buflen - len, "he_mac=");
		if (os_snprintf_error(buflen - len, ret))
			return len;
		len += ret;
		for (i = 0; i < sizeof(sta->he_capabilities->he_mac_capab_info); i++) {
			ret = os_snprintf(buf + len, buflen - len, "%02x%s", *(he + i),
					  i + 1 < sizeof(sta->he_capabilities->he_mac_capab_info) ? " " : "\n");
			if (os_snprintf_error(buflen - len, ret))
				return len;
			len += ret;
		}

		he = (u8*) &sta->he_capabilities->he_phy_capab_info;
		ret = os_snprintf(buf + len, buflen - len, "he_phy=");
		if (os_snprintf_error(buflen - len, ret))
			return len;
		len += ret;
		for (i = 0; i < sizeof(sta->he_capabilities->he_phy_capab_info); i++) {
			ret = os_snprintf(buf + len, buflen - len, "%02x%s", *(he + i),
					  i + 1 < sizeof(sta->he_capabilities->he_phy_capab_info) ? " " : "\n");
			if (os_snprintf_error(buflen - len, ret))
				return len;
			len += ret;
		}

		he = (u8*) &sta->he_capabilities->he_txrx_mcs_support;
		ret = os_snprintf(buf + len, buflen - len, "he_mcs_nss=");
		if (os_snprintf_error(buflen - len, ret))
			return len;
		len += ret;
		for (i = 0; i < (sizeof(sta->he_capabilities->he_txrx_mcs_support)/3); i++) {
			ret = os_snprintf(buf + len, buflen - len, "%02x ", *(he + i));
			if (os_snprintf_error(buflen - len, ret))
				return len;
			len += ret;
		}
		he += sizeof(sta->he_capabilities->he_txrx_mcs_support)/3;
		for (i = 0; i < (sizeof(sta->he_capabilities->he_txrx_mcs_support)/3); i++) {
			ret = os_snprintf(buf + len, buflen - len, "%02x%s", *(he + i),
					  i + 1 < (sizeof(sta->he_capabilities->he_txrx_mcs_support)/3) ? " " : "\n");
			if (os_snprintf_error(buflen - len, ret))
				return len;
			len += ret;
		}

		he = (u8*) &sta->he_capabilities->he_ppe_thresholds;
		ret = os_snprintf(buf + len, buflen - len, "he_ppe_thresholds=");
		if (os_snprintf_error(buflen - len, ret))
			return len;
		len += ret;
		for (i = 0; i < sizeof(sta->he_capabilities->he_ppe_thresholds); i++) {
			ret = os_snprintf(buf + len, buflen - len, "%02x%s", *(he + i),
					  i + 1 < sizeof(sta->he_capabilities->he_ppe_thresholds) ? " " : "\n");
			if (os_snprintf_error(buflen - len, ret))
				return len;
			len += ret;
		}
			      ...
			      size_t buflen)
{
	...
	ret = os_snprintf(buf + len, buflen - len,
			  ...
			  iface->conf->ieee80211ax && !hapd->conf->disable_11ax,

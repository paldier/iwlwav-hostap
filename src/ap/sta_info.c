...
{
	int set_beacon = 0;
	Boolean is_recovery = (hapd->iface->stas_free_reason == WLAN_STA_FREE_REASON_RECOVERY);
			   ...
			   int authorized)
{
	...
	unsigned int rrm_cap = 0, band_2g_capable = 0, band_5g_capable = 0;
#ifdef CONFIG_P2P
	u8 addr[ETH_ALEN];
	u8 ip_addr_buf[4];
#endif /* CONFIG_P2P */
	...
	if (authorized) {
		...
		band_2g_capable = band_5g_capable = 0;
		if (sta->supported_channels_len > 0) {
			for (i = 0; i < sta->supported_channels_len; i++) {
				if (sta->supported_channels[i] < FIRST_5G_CHAN)
					band_2g_capable = 1;
				else
					band_5g_capable = 1;
		...
		ret = os_snprintf(sta_extra_cap + len, STA_EXTRA_CAP_LEN - len, " rrm_supported=%d band_2g_capable=%d band_5g_capable=%d ",
			rrm_cap, band_2g_capable, band_5g_capable);
		...
		ret = os_snprintf(sta_extra_cap + len, STA_EXTRA_CAP_LEN - len,
		...
		len += ret;
		ret = os_snprintf(sta_extra_cap + len, STA_EXTRA_CAP_LEN - len, "rrm_cap=%02x %02x %02x %02x %02x",
		...
		ret = os_snprintf(sta_extra_cap, STA_EXTRA_CAP_LEN,
				" reason=%d source=%d type=%d", reason, sta->disconnect_source, type);
		if (os_snprintf_error(STA_EXTRA_CAP_LEN, ret)) {
			goto free_alloc;
		}
...
#define STA_FLAGS_AX_STR "%s%s"
	...
	buf[0] = '\0';
	res = os_snprintf(buf, buflen, "%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s" STA_FLAGS_AX_STR,

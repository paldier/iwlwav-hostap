...
u8 * hostapd_eid_ht_capabilities(struct hostapd_data *hapd, u8 *eid)
{
	...
	*pos++ = sizeof(*cap);
	...
	if(hapd->iconf->ht_mcs_set[0]) {
		os_memcpy(cap->supported_mcs_set, &hapd->iconf->ht_mcs_set[0], sizeof(cap->supported_mcs_set));
	} else {
	...
	if (elems->ht_capabilities) {
		...
		if (le_to_host16(ht_cap->ht_capabilities_info) &
			...
			hostapd_logger(hapd, mgmt->sa,
				       HOSTAPD_MODULE_IEEE80211,
				       HOSTAPD_LEVEL_INFO,
				       "Switching to 20 MHz operation due to OBSS beacon reception at rssi %d", rssi_level);
		      ...
		      const u8 *ht_operation)
{
	...
	if (sta->ht_operation == NULL) {
		sta->ht_operation =
			os_zalloc(sizeof(struct ieee80211_ht_operation));
		if (sta->ht_operation == NULL)
			return WLAN_STATUS_UNSPECIFIED_FAILURE;
	}

	os_memcpy(sta->ht_operation, ht_operation,

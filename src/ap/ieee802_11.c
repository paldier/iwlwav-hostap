...
static void handle_auth(struct hostapd_data *hapd,
			...
			int rssi, int from_queue, int snr_db);
			...
			int rssi, int from_queue, int snr_db)
{
	...
	softblock_res = hostapd_check_softblock(hapd, mgmt->sa, &resp, WLAN_FC_STYPE_AUTH,
				snr_db, &msg_dropped);
	...
	if (auth_alg == WLAN_AUTH_SAE && !from_queue &&
		 ...
		 * SAE exchange. */
		auth_sae_queue(hapd, mgmt, len, rssi,snr_db);
		return;
	}
#endif /* CONFIG_SAE */
			   ...
			   struct ieee802_11_elems *elems)
{
	...
	while (remain > 1) { /* parse by pair of bytes: chan and range */
		...
		if (nof_max < (filled + range)) {
			hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE80211,
					HOSTAPD_LEVEL_DEBUG,
					"Supported channels number exceeds %u",
					nof_max);
			return WLAN_STATUS_UNSPECIFIED_FAILURE;
		}
			   ...
			   const u8 *ies, size_t ies_len, int reassoc)
{
	...
	if (hapd->iconf->ieee80211ac) {
		...
		if (sta->vht_capabilities != NULL) { /* set by copy_sta_vht_capab, don't reorder */
			resp = copy_sta_vht_operation(hapd, sta, elems.vht_operation);
			if (resp != WLAN_STATUS_SUCCESS)
				return resp;
		}
	...
	if ((elems.multi_ap) ||
		...
		(hapd->conf->vendor_4addr && elems.brcom_ie && (elems.brcom_ie[VENDOR_BRCOM_FLAGS1_OFFSET] & VENDOR_BRCOM_DWDS_CAPABLE))
		)
			   ...
			   const u8 *ies, size_t ies_len, int rssi)
{
	...
	if (sta && hapd->iconf->assoc_rsp_rx_mcs_mask) {
	...
	if ((hapd->iconf->ieee80211ac && !hapd->conf->disable_11ac) ||
		(hapd->conf->vendor_vht && (sta && sta->flags & WLAN_STA_VENDOR2_VHT))) {
			 ...
			 int reassoc, int rssi)
{
	...
	if (hostapd_check_softblock(hapd, mgmt->sa, &resp, WLAN_FC_STYPE_ASSOC_REQ, 0, NULL)
		== HOSTAPD_MULTI_AP_BLACKLIST_FOUND) {
...
static void _notify_action_frame(struct hostapd_data *hapd, const u8 *action_frame,
			      size_t length)
{
			 ...
			 unsigned int freq, int ssi_signal)
{
	...
	if (hapd->iconf->notify_action_frame)
		_notify_action_frame(hapd, (const u8 *)mgmt, len);
		    ...
		    struct hostapd_frame_info *fi)
{
	...
	if (stype == WLAN_FC_STYPE_PROBE_REQ) {
		handle_probe_req(hapd, mgmt, len, ssi_signal, snr_db);
		return 1;
	}
	...
	case WLAN_FC_STYPE_AUTH:
		...
		handle_auth(hapd, mgmt, len, ssi_signal, 0, snr_db);
		ret = 1;
		break;

					    ...
					    int show_errors)
{
	...
	case OUI_MICROSOFT:
		...
		case WMM_OUI_TYPE:
			...
			case WMM_OUI_SUBTYPE_INFORMATION_ELEMENT:
				if (elen != sizeof(struct wmm_information_element))
					return -1;
				elems->wmm = pos;
				elems->wmm_len = elen;
				break;
			case WMM_OUI_SUBTYPE_PARAMETER_ELEMENT:
				if (elen != sizeof(struct wmm_parameter_element))
					return -1;
				/*
			...
			case WMM_OUI_SUBTYPE_TSPEC_ELEMENT:
				if (elen != sizeof(struct wmm_tspec_element))
					return -1;
				elems->wmm_tspec = pos;
	...
	case OUI_WFA:
		...
		case HS20_INDICATION_OUI_TYPE:
			/* Hotspot 2.0 */
			if (elen > HS20_INDICATION_OUI_TYPE_MAX_LEN)
				return -1;
			elems->hs20 = pos;
			elems->hs20_len = elen;
			break;
		...
		case HS20_ROAMING_CONS_SEL_OUI_TYPE:
			...
			if (elen > HS20_ROAMING_CONS_SEL_OUI_TYPE_MAX_LEN)
				return -1;
				      ...
				      int show_errors)
{
	...
	case WLAN_EID_EXT_FILS_REQ_PARAMS:
		if (elen < 3 || elen > WLAN_EID_EXT_FILS_REQ_PARAMS_MAX_LEN)
			break;
	...
	case WLAN_EID_EXT_PASSWORD_IDENTIFIER:
		...
		break;
	/* Set ptr pass Ext ID as it has already been parsed above */
	case WLAN_EID_EXT_HE_CAPABILITIES:
		{
		struct ieee80211_he_capabilities capab;
		if (elen < (sizeof(capab.he_mac_capab_info)) +
			(sizeof(capab.he_phy_capab_info)) +
			((sizeof(capab.he_txrx_mcs_support))/3)) {
			wpa_printf(MSG_WARNING, "Ignoring STA HE capabilities due to wrong size: %d", elen);
			break;
		}
		wpa_printf(MSG_EXCESSIVE, "got WLAN_EID_HE_CAP_EXT elen:%d", elen);
		elems->pc_he_capabilities = pos;
		elems->he_capabilities_len_from_sta = elen;
		break;
		}
	case WLAN_EID_EXT_HE_OPERATION:
		if (elen > sizeof(struct ieee80211_he_operation)) {
			wpa_printf(MSG_WARNING, "Ignoring STA HE operation IE due to wrong size: %d", elen);
			break;
		}
		wpa_printf(MSG_EXCESSIVE, "got WLAN_EID_HE_OPERATION_EXT elen:%d", elen);
		elems->pc_he_operation = pos;
		elems->he_operation_len_from_sta = elen;
		break;
	case WLAN_EID_EXT_HE_MU_EDCA_PARAMS:
		if (elen > sizeof(struct ieee80211_he_mu_edca_parameter_set)) {
			wpa_printf(MSG_WARNING, "Ignoring STA HE MU EDCA IE due to wrong size: %d", elen);
			break;
		}
		wpa_printf(MSG_EXCESSIVE, "got WLAN_EID_HE_MU_EDCA_EXT elen:%d", elen);
		elems->pc_he_mu_edca = pos;
		elems->he_mu_edca_len_from_sta = elen;
		break;
	case WLAN_EID_EXT_HE_SPATIAL_REUSE_SET:
		if (elen > sizeof(struct ieee80211_he_spatial_reuse_parameter_set)){
			wpa_printf(MSG_WARNING, "Ignoring STA HE spatial reuse IE due to wrong size: %d", elen);
			break;
		}
		wpa_printf(MSG_EXCESSIVE, "got WLAN_EID_EXT_HE_SPATIAL_REUSE_SET elen:%d", elen);
		elems->pc_he_spatial_reuse_set = pos;
		elems->he_spatial_reuse_set_len_from_sta = elen;
		break;
				...
				int show_errors)
{
	...
	for_each_element(elem, start, len) {
		...
		case WLAN_EID_SUPP_RATES:
			if (elen > WLAN_EID_SUPP_RATES_MAX_LEN)
				break;
		...
		case WLAN_EID_CHALLENGE:
			if (elen > WLAN_EID_CHALLENGE_MAX_LEN)
				break;
			elems->challenge = pos;
		...
		case WLAN_EID_MESH_CONFIG:
			if (elen != WLAN_EID_MESH_CONFIG_LEN)
				break;
			elems->mesh_config = pos;
			elems->mesh_config_len = elen;
			break;
		case WLAN_EID_MESH_ID:
			if (elen > WLAN_EID_MESH_ID_MAX_LEN)
				break;
			elems->mesh_id = pos;
			elems->mesh_id_len = elen;
			break;
		case WLAN_EID_PEER_MGMT:
			if (elen > WLAN_EID_PEER_MGMT_MAX_LEN)
				break;
			elems->peer_mgmt = pos;
		...
		case WLAN_EID_LINK_ID:
			if (elen != WLAN_EID_LINK_ID_LEN)
				break;
			elems->link_id = pos;
			break;
		case WLAN_EID_INTERWORKING:
			if (elen > WLAN_EID_INTERWORKING_MAX_LEN)
				break;
		...
		case WLAN_EID_EXT_CAPAB:
			if (elen > WLAN_EID_EXT_CAPAB_MAX_LEN)
				break;
			elems->ext_capab = pos;
		...
		case WLAN_EID_RRM_ENABLED_CAPABILITIES:
			if (elen != WLAN_EID_RRM_ENABLED_CAPABILITIES_LEN)
				break;
			elems->rrm_enabled = pos;
			elems->rrm_enabled_len = elen;
			break;
		case WLAN_EID_MLTIPLE_BSSID_INDEX:
		...
		case WLAN_EID_FILS_INDICATION:
			...
			break;
		case WLAN_EID_DILS:
			if (elen < 2 || elen > WLAN_EID_DILS_MAX_LEN)
				break;
			elems->dils = pos;
			elems->dils_len = elen;
			break;

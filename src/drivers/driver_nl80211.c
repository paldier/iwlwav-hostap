			 ...
			 void *valid_data)
{
	...
	setsockopt(nl_socket_get_fd(nl_handle), SOL_NETLINK,
		   NETLINK_EXT_ACK, (const void *)&opt, sizeof(opt));

	/* try to set NETLINK_CAP_ACK to 1, ignoring errors */
	opt = 1;
	setsockopt(nl_socket_get_fd(nl_handle), SOL_NETLINK,
		   NETLINK_CAP_ACK, (const void *)&opt, sizeof(opt));
...
static int nl80211_set_bss(struct i802_bss *bss, struct wpa_driver_ap_params *params)
{
	struct wpa_driver_nl80211_data *drv = bss->drv;
	struct nl_msg *msg;

	if (!(msg = nl80211_bss_msg(bss, 0, NL80211_CMD_SET_BSS)) ||
	    (params->cts_protect >= 0 &&
	     nla_put_u8(msg, NL80211_ATTR_BSS_CTS_PROT, params->cts_protect)) ||
	    (params->preamble >= 0 &&
	     nla_put_u8(msg, NL80211_ATTR_BSS_SHORT_PREAMBLE, params->preamble)) ||
	    (params->short_slot_time >= 0 &&
	     nla_put_u8(msg, NL80211_ATTR_BSS_SHORT_SLOT_TIME, params->short_slot_time)) ||
	    (params->ht_opmode >= 0 &&
	     nla_put_u16(msg, NL80211_ATTR_BSS_HT_OPMODE, params->ht_opmode)) ||
	    (params->isolate >= 0 &&
	...
	if (!params->set || (params->flags & WPA_STA_TDLS_PEER) ||
		...
		if (params->he_capabilities) {
				wpa_hexdump(MSG_DEBUG, "  * he_capabilities",
										...
										sizeof(*params->he_capabilities));
				if (nla_put(msg, NL80211_ATTR_HE_CAPABILITY,
										sizeof(*params->he_capabilities),
										params->he_capabilities))
								goto fail;
		}
		if (params->he_operation) {
				wpa_hexdump(MSG_DEBUG, "  * he_operation",
										(u8 *) params->he_operation,
										sizeof(*params->he_operation));
				if (nla_put(msg, NL80211_ATTR_HE_OPERATION,
										sizeof(*params->he_operation),
										params->he_operation))
								goto fail;
		}
...
int nl80211_set_sta_softblock_thresholds(void *priv,
        const struct multi_ap_blacklist *entry, const u8 remove)
{
  int ret;
  struct intel_vendor_blacklist_cfg blacklist;
  ...
  ret = nl80211_vendor_cmd(priv, OUI_LTQ,
    LTQ_NL80211_VENDOR_SUBCMD_SET_SOFTBLOCK_THRESHOLDS, (u8*) &blacklist, sizeof(blacklist),
    NULL);
...
int nl80211_set_mbssid_num_vaps_in_group(void *priv, const u8 mbssid_num_vaps_in_group)
{
  int ret = nl80211_vendor_cmd(priv, OUI_LTQ, LTQ_NL80211_VENDOR_SUBCMD_SET_MBSSID_NUM_VAPS_IN_GROUP,
    &mbssid_num_vaps_in_group, 1, NULL);
...
int nl80211_get_he_operation_info(void *priv, struct ieee80211_he_operation *he_operation)
{
#define KERNEL_HE_OPER_SIZE 6
  int res = 0;
  size_t min_size;
  struct wpabuf *rsp_he_oper;

  if (NULL == he_operation) {
    return -EFAULT;
  }

  rsp_he_oper = wpabuf_alloc(NLA_ALIGN(sizeof(struct nlattr) + sizeof(*he_operation)));

  if (!rsp_he_oper) {
    return -ENOBUFS;
  }

  res = nl80211_vendor_cmd(priv, OUI_LTQ, LTQ_NL80211_VENDOR_SUBCMD_GET_HE_OPERATION,
                                                                     NULL, 0, rsp_he_oper);
  if (res) {
    wpa_printf(MSG_ERROR, "nl80211: sending/receiving HE_OPERATION failed: %i "
         "(%s)", res, strerror(-res));
    goto out;
  }

  if (rsp_he_oper->used < KERNEL_HE_OPER_SIZE) {
    res = -EMSGSIZE;
    wpa_printf(MSG_ERROR, "nl80211: driver returned %d bytes instead of at least %d",
                                       rsp_he_oper->used, KERNEL_HE_OPER_SIZE);
    goto out;
  }

  min_size = MIN(rsp_he_oper->used, sizeof(*he_operation));
  memcpy(he_operation, rsp_he_oper->buf, min_size);
  wpa_hexdump(MSG_DEBUG, "Received a new HE_OPERATION", he_operation, min_size);

out:
  wpabuf_free(rsp_he_oper);
  return res;
}

int nl80211_set_he_operation_info(void *priv, struct ieee80211_he_operation *he_operation)
{
	int ret;

	wpa_printf(MSG_ERROR, " nl80211: sending SET_HE_OPERATION ");

	ret = nl80211_vendor_cmd(priv, OUI_LTQ, LTQ_NL80211_VENDOR_SUBCMD_SET_HE_OPERATION,
							(u8*) he_operation, sizeof(*he_operation), NULL);

	if (ret < 0) {
		wpa_printf(MSG_ERROR, "nl80211: sending SET_HE_OPERATION failed: %i (%s)",
				   ret, strerror(-ret));
	}
	return ret;
}
...
int nl80211_get_radio_info(void *priv, struct intel_vendor_radio_info *radio_info)
{
  int ret;
  struct wpabuf *rsp;
...
int nl80211_send_atf_quotas(void *priv, struct intel_vendor_atf_quotas* atf_quotas)
{
  int ret = nl80211_vendor_cmd(priv, OUI_LTQ, LTQ_NL80211_VENDOR_SUBCMD_SET_ATF_QUOTAS,
		  (u8*)atf_quotas, sizeof(struct intel_vendor_atf_quotas) + atf_quotas->data_len, NULL);
...
int nl80211_unconnected_sta(void *priv, struct intel_vendor_unconnected_sta_req_cfg *req)
{
  int res = 0;
...
static int nl80211_set_param(void *priv, const char *param)
{
	...
	if (pos) {
		...
		val = atoi(pos);
		if ((val >= SCAN_TIMEOUT_MIN) && (val <= SCAN_TIMEOUT_MAX)) bss->scan_timeout = val;
			else wpa_printf(MSG_ERROR, "scan_timeout %d out of range %d..%d, using default",
...
int nl80211_set_zwdfs_antenna(void *priv, int enable)
{
	int ret = nl80211_vendor_cmd(priv, OUI_LTQ, LTQ_NL80211_VENDOR_SUBCMD_SET_ZWDFS_ANT,
		(u8*) &enable, sizeof(enable), NULL);
...
const struct wpa_driver_ops wpa_driver_nl80211_ops = {
	...
	.set_sta_softblock_thresholds = nl80211_set_sta_softblock_thresholds,

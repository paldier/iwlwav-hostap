...
static void mlme_event_mgmt(struct i802_bss *bss,
			    ...
			    struct nlattr *snr, const u8 *frame, size_t len)
{
	...
	wpa_printf(MSG_DEBUG,
		   ...
		   " freq=%d ssi_signal=%d snr_db=%d fc=0x%x seq_ctrl=0x%x stype=%u (%s) len=%u",
		   MAC2STR(mgmt->da), MAC2STR(mgmt->sa), MAC2STR(mgmt->bssid),
		   rx_freq, ssi_signal, snr_db, fc,
...
static void mlme_event(struct i802_bss *bss,
		       ...
		       struct nlattr *snr, struct nlattr *wmm,struct nlattr *req_ie)
{
	struct wpa_driver_nl80211_data *drv = bss->drv;
	const u8 *data;
	size_t len;
	...
	case NL80211_CMD_FRAME:
		mlme_event_mgmt(bss, freq, sig, snr, nla_data(frame),
				nla_len(frame));
		break;
...
static void ltq_nl80211_handle_softblock_drop(struct wpa_driver_nl80211_data *drv,
				const u8 *data, size_t len)
{
	wpa_printf(MSG_INFO, "nl80211: Receive LTQ vendor event:Softblock Drop, ctx=%p",
		drv->ctx);

	drv_event_ltq_softblock_drop(drv->ctx, data, len);
}
    ...
    case LTQ_NL80211_VENDOR_EVENT_CSA_RECEIVED:
      ...
      break;
		...
		case LTQ_NL80211_VENDOR_EVENT_SOFTBLOCK_DROP:
			ltq_nl80211_handle_softblock_drop(drv, data, len);
			break;
		default:

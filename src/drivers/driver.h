...
//  u32	band;
  ...
  Boolean overlap80;
  Boolean overlap160;
  Boolean overlap_vht_he;
  u32	primary_on_overlap_4080160;
  u32	intolerant40;
  ...
  struct os_reltime ts_overlap160;     // Timestamp of overlapping 160 MHz detected
...
#define WPA_DRIVER_FLAGS_4WAY_HANDSHAKE_8021X		0x00000008ULL
...
struct wpa_driver_ops {
  ...
  int (*sync_done)(void *priv);
	...
	/**
	* set_sta_softblock_thresholds - Add/Modify MAC address to blacklist
	* @priv: Private driver interface data
	* @entry: pointer to multi_ap_blacklist structure
	* @remove: 1 - remove address from blacklist, 0 - add address to blacklist
	* Returns: 0 on success, -1 on failure 
	*/
	int (*set_sta_softblock_thresholds)(void *priv, const struct multi_ap_blacklist *entry,
		const u8 remove);
   ...
   * Returns: 0 on success, -1 on failure
   */
  int (*get_vap_measurements)(void *priv, struct intel_vendor_vap_info *vap_info);
   ...
   * Returns: 0 on success, -1 on failure
   */
  int (*get_radio_info)(void *priv, struct intel_vendor_radio_info *radio_info);
   ...
   * Returns: 0 on success, -1 on failure
   */
  int (*send_atf_quotas)(void *priv, struct intel_vendor_atf_quotas* atf_quotas);
  ...
  int (*set_antenna)(void *priv, u32 tx_ant, u32 rx_ant);
	...
	/**
	 * get_he_operation_info - Get HE Operation element
	 * @priv: Private driver interface data
	 * @he_operation - HE operation element struct
	 * Returns: 0 on success, -1 on failure
	 */
	int(*get_he_operation_info)(void *priv, struct ieee80211_he_operation *he_operation);

	/**
	 * set_he_operation_info - Set HE Operation element
	 * @priv: Private driver interface data
	 * @he_operation - HE operation element struct
	 * Returns: 0 on success, -1 on failure
	 */
	...
	* @priv: Private driver interface data
	* @enable: Flag indicating enable/disable ZWDFS antenna
	* Returns: 0 on success, -1 on failure
	*/
	int (*set_zwdfs_antenna)(void *priv, int enable);
...
enum wpa_event_type {
	...
	/**
	 * EVENT_CH_SWITCH_STARTED - AP or GO will switch channels soon
	 *
	 * Described in wpa_event_data.ch_switch
	 * */
...
union wpa_event_data {
	...
	struct rx_mgmt {
		...
		int ssi_signal;
		/**
		 * snr_db - SNR in dBm (or 0 if not available)
		 */
		int snr_db;		
	} rx_mgmt;
	...
	/* Data for EVENT_LTQ_FLUSH_STATIONS_RX events */
	...
	/* Data for EVENT_LTQ_SOFTBLOCK_DROP events */
...
static inline void drv_event_ltq_softblock_drop(void *ctx, const u8 *data, size_t data_len)
{
	union wpa_event_data event;
	os_memset(&event, 0, sizeof(event));
	os_memcpy(&event.msg_dropped, data, data_len);
	wpa_supplicant_event(ctx, EVENT_LTQ_SOFTBLOCK_DROP, &event);
}
...
static inline void drv_event_ltq_unconnected_sta(void *ctx, const u8 *data, size_t data_len)
{
   union wpa_event_data event;
   os_memset(&event, 0, sizeof(event));
   memcpy(&event.unconnected_sta, data, data_len);
...
static inline void drv_event_ltq_csa_received (void *ctx, const u8 *data, size_t data_len)
{
  union wpa_event_data event;
  os_memset(&event, 0, sizeof(event));
  memcpy(&event.csa_received, data, data_len);
...
static inline void drv_event_ltq_radar (void *ctx, const u8 *data, size_t data_len)
{
	union wpa_event_data event;
	os_memset(&event, 0, sizeof(event));
	memcpy(&event.radar, data, data_len);

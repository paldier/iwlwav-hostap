			     ...
			     struct nlattr *tx_bf)
{
	...
	if (tx_bf)
		mode->ht_tx_bf_capab = nla_get_u32(tx_bf);
}
...
static int phy_info_he_caps(struct hostapd_hw_modes *mode, struct nlattr *tb)
{
	struct nlattr *tb_iftype[NL80211_BAND_IFTYPE_ATTR_MAX + 1];
	struct nlattr *nl_he_cap;
	int rem_iftype;

	if (tb == NULL)
		return NL_OK;

	nla_for_each_nested(nl_he_cap, tb, rem_iftype) {
		nla_parse(tb_iftype, NL80211_BAND_IFTYPE_ATTR_MAX, nla_data(nl_he_cap), nla_len(nl_he_cap), NULL);

		if (tb_iftype[NL80211_BAND_IFTYPE_ATTR_HE_CAP_MAC]) {
			os_memcpy(&mode->he_capab.he_mac_capab_info,
								nla_data(tb_iftype[NL80211_BAND_IFTYPE_ATTR_HE_CAP_MAC]),
								MIN(sizeof(mode->he_capab.he_mac_capab_info), nla_len(tb_iftype[NL80211_BAND_IFTYPE_ATTR_HE_CAP_MAC])));
		}

		if (tb_iftype[NL80211_BAND_IFTYPE_ATTR_HE_CAP_PHY]) {
			os_memcpy(&mode->he_capab.he_phy_capab_info,
								nla_data(tb_iftype[NL80211_BAND_IFTYPE_ATTR_HE_CAP_PHY]),
								 MIN(sizeof(mode->he_capab.he_phy_capab_info), nla_len(tb_iftype[NL80211_BAND_IFTYPE_ATTR_HE_CAP_PHY])));
		}
		if (tb_iftype[NL80211_BAND_IFTYPE_ATTR_HE_CAP_PPE]) {
			os_memcpy(&mode->he_capab.he_ppe_thresholds,
								nla_data(tb_iftype[NL80211_BAND_IFTYPE_ATTR_HE_CAP_PPE]),
								 MIN(sizeof(mode->he_capab.he_ppe_thresholds), nla_len(tb_iftype[NL80211_BAND_IFTYPE_ATTR_HE_CAP_PPE])));
		}

		if (tb_iftype[NL80211_BAND_IFTYPE_ATTR_HE_CAP_MCS_SET]) {
			os_memcpy(&mode->he_capab.he_txrx_mcs_support, nla_data(tb_iftype[NL80211_BAND_IFTYPE_ATTR_HE_CAP_MCS_SET]),
								 MIN(sizeof(mode->he_capab.he_txrx_mcs_support), nla_len(tb_iftype[NL80211_BAND_IFTYPE_ATTR_HE_CAP_MCS_SET])));
		}
	}
...
static int phy_info_band(struct phy_info_arg *phy_info, struct nlattr *nl_band)
{
	...
	if (phy_info->last_mode != nl_band->nla_type) {
		...
		/* Unsupported HE MCS NSS defined by default */
		os_memset(&mode->he_capab.he_txrx_mcs_support, 0xff,
										sizeof(mode->he_capab.he_txrx_mcs_support));
	...
	phy_info_ht_capa(mode, tb_band[NL80211_BAND_ATTR_HT_CAPA],
			 ...
			 tb_band[NL80211_BAND_ATTR_HT_TX_BF_CAPA]);
	phy_info_vht_capa(mode, tb_band[NL80211_BAND_ATTR_VHT_CAPA],
			  tb_band[NL80211_BAND_ATTR_VHT_MCS_SET]);

	phy_info_he_caps(mode, tb_band[NL80211_BAND_ATTR_IFTYPE_DATA]);

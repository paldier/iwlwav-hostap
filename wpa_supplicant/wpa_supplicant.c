...
static int hostapd_reload(struct wpa_supplicant *wpa_s, struct wpa_bss *bss)
{
	...
	os_snprintf(cmd, sizeof(cmd), "UPDATE_RELOAD channel=%d sec_chan=%d "
		     "center_freq1=%d center_freq2=%d bandwidth=%d",
		     channel, sec_chan, si.center_frq1, si.center_frq2, si.chanwidth);
	...
	if (state == WPA_COMPLETED && wpa_s->new_connection) {
		...
		if (ssid && ssid->id_str)
			wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_CONNECTED
					MACSTR " id=%d id_str=%s%s", MAC2STR(wpa_s->bssid),
					ssid ? ssid->id : -1, ssid->id_str,
					fils_hlp_sent ? " FILS_HLP_SENT" : "");
		else
			wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_CONNECTED
					MACSTR " id=%d%s", MAC2STR(wpa_s->bssid), ssid ? ssid->id : -1,
					fils_hlp_sent ? " FILS_HLP_SENT" : "");

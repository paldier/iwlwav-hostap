			...
			const char *arg2)
{
	...
	os_strlcpy(arg, arg1, len);
	os_strlcat(arg, " ", len);
	os_strlcat(arg, arg2, len);
...
/*
 * OS dependent validation for the $HOME path.
 * Input parameter char *in_path must be dynamically allocated
 * prior to the function call. The function takes care for the *in_path
 * deallocation.
 * Function returns pointer to dynamically allocated validated real path string
 * with 0 character at the end. Caller is responsible to free up function result
 * with os_free() call.
 */
static char* wpa_cli_home_path_validate(char *in_path)
{
	char *real_path;
	...
	real_path = realpath(in_path, NULL);
	os_free(in_path);
	...
	if(real_path[0] != '/')
		goto err;
	if (strstr(real_path, "/bin") == real_path)
		goto err;
	if (strstr(real_path, "/dev") == real_path)
		goto err;
	if (strstr(real_path, "/etc") == real_path)
		goto err;
	if (strstr(real_path, "/sys") == real_path)
		goto err;
	if (strstr(real_path, "/proc") == real_path)
		goto err;

	return real_path;

err:
	printf("HOME path validation error \n");
	os_free(real_path);
	return NULL;
}
...
static void start_edit(void)
{
	...
	if (home) {
		...
		if (hfile) {
			os_strlcpy(hfile, home, hfile_len);
			os_strlcat(hfile, "/", hfile_len);
			hfile = wpa_cli_home_path_validate(hfile);
			if (hfile == NULL) {
				eloop_terminate();
				return;
			}
			os_strlcat(hfile, fname, hfile_len);
		}
	}

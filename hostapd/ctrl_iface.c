...
static void hostapd_ctrl_iface_config_update(struct hostapd_config *conf, char* reload_opts)
{
	...
	if (str) {
		str += strlen("center_freq1=");
		val = strtol(str, NULL, 10);
		ieee80211_freq_to_chan(val, &conf->vht_oper_centr_freq_seg0_idx);
	}

	str = os_strstr(reload_opts, "center_freq2=");
	if (str) {
		str += strlen("center_freq2=");
		val = strtol(str, NULL, 10);
		ieee80211_freq_to_chan(val, &conf->vht_oper_centr_freq_seg1_idx);
	}

	str = os_strstr(reload_opts, "bandwidth=");
	if (str) {
		str += strlen("bandwidth=");
		val = strtol(str, NULL, 10);
			...
			struct hostapd_data *hapd, char *input_data)
{
	...
	len = os_strlen(input_data);
	if (len < 3) /* minimum theoretical length of key=value pair */
		return -1;
	...
	for (pos = 0, key_no = 0; pos < len; pos = val_end + 1 - input_data, key_no++) {
		...
		if (*val_start == '"') val_end = strchr(val_start + 1, '"') + 1;
		else if (*val_start == 'P' && val_start[1] == '"') val_end = strchr(val_start + 2, '"') + 1;
		else {
	...
	/* initialize the vap */
	eloop_cancel_timeout(terminate_hostapd, 0, hapd);
...
#ifdef NEED_AP_MLME
static Boolean ctrl_iface_chan_is_dfs_usable(struct hostapd_iface *iface, int frequency)
{
	int i;
		...
		char *buf, size_t buflen)
{
	...
	if (pos) {
		...
		if (last_indication != 0 && last_indication != 1) {
			wpa_printf(MSG_ERROR,
						...
						char *cmd)
{
	...
	if (enable) {
		...
		conf->start_disabled = 0;
		hostapd_disable_iface(iface);
		hostapd_enable_iface(iface);
		wpa_msg(hapd->msg_ctx, MSG_INFO, ZWDFS_ANT_ENABLE);
	} else {
		if (hostapd_drv_set_zwdfs_antenna(hapd, enable) < 0) {
			wpa_printf(MSG_ERROR, "Failed to disable ZWDFS antenna");
			return -1;
		}
		conf->start_disabled = 1;
		hostapd_disable_iface(iface);
		hostapd_enable_iface(iface);
		wpa_msg(hapd->msg_ctx, MSG_INFO, ZWDFS_ANT_DISABLE);
	}
...
int hostapd_ctrl_iface_sta_softblock(struct hostapd_data *hapd, const char *cmd)
{
	...
	if (pos) {
		...
		snr = atoi(pos);
		if (snr < 0) {
			wpa_printf(MSG_ERROR, "sta_softblock: invalid snrProbeLWM");
			return -1;
		}
		entry.snrProbeLWM = snr;
	} else {
		wpa_printf(MSG_ERROR, "sta_softblock: missing parameter(s)");
		return -1;
	}
	...
	if (pos) {
		...
		snr = atoi(pos);
		if (snr < 0) {
			wpa_printf(MSG_WARNING, "sta_softblock: invalid snrAuthLWM");
			return -1;
		}
		entry.snrAuthLWM = snr;
	} else {
		wpa_printf(MSG_ERROR, "sta_softblock: missing parameter(s)");
		return -1;
	}
...
int hostapd_ctrl_iface_sta_steer(struct hostapd_data *hapd, const char *cmd)
{
	...
	if (!sta || !sta->btm_supported) {
		...
		os_snprintf(buf, sizeof(buf), MACSTR " %s", MAC2STR(sta_addr),
...
//  struct mtlk_wssa_drv_peer_rate_info1* rate_info = NULL;
  ...
  ret = os_snprintf(buf + len, buflen - len, "SNR=%d %d %d %d\n",
  ...
  if (sta_info.RateInfoFlag) { /* available */
    ret = os_snprintf(buf + len, buflen - len, "PhyMode=%d\n", sta_info.RatePhyMode);
    if (ret >= buflen - len || ret < 0)
      return len;
    len += ret;
    ret = os_snprintf(buf + len, buflen - len, "Nss=%d\n", sta_info.RateNss);
    if (ret >= buflen - len || ret < 0)
      return len;
    len += ret;
    ret = os_snprintf(buf + len, buflen - len, "CbwMHz=%d\n", sta_info.RateCbwMHz);
    if (ret >= buflen - len || ret < 0)
      return len;
    len += ret;
  }

  ret = os_snprintf(buf + len, buflen - len, "TxStbcMode=%d\n", sta_info.TxStbcMode);
  if (ret >= buflen - len || ret < 0)
    return len;
  len += ret;

  ret = os_snprintf(buf + len, buflen - len, "pairwise_cipher=%x\n", sta->wpa_sm ? wpa_auth_get_pairwise(sta->wpa_sm) : 0);
  if (ret >= buflen - len || ret < 0)
    return len;
  len += ret;

  ret = os_snprintf(buf + len, buflen - len, "wpa_key_mgmt=%x\n", sta->wpa_sm ? wpa_auth_sta_key_mgmt(sta->wpa_sm) : 0);
  if (ret >= buflen - len || ret < 0)
    return len;
  len += ret;
  ...
  const char *cmd, char *buf, size_t buflen)
{
  ...
  ret = os_snprintf(buf + len, buflen - len, "BytesSent=%llu\n",
		  vap_info.traffic_stats.BytesSent);
  if (ret >= buflen - len || ret < 0)
    return len;
  len += ret;
  ret = os_snprintf(buf + len, buflen - len, "BytesReceived=%llu\n",
		  vap_info.traffic_stats.BytesReceived);
  if (ret >= buflen - len || ret < 0)
    return len;
  len += ret;
  ret = os_snprintf(buf + len, buflen - len, "PacketsSent=%llu\n",
		  vap_info.traffic_stats.PacketsSent);
  if (ret >= buflen - len || ret < 0)
    return len;
  len += ret;
  ret = os_snprintf(buf + len, buflen - len, "PacketsReceived=%llu\n",
  ...
  const char *cmd, char *buf, size_t buflen)
{
  ...
  for (i = 0; i < iface->num_candidates; i++) {
    ...
    if (candidates[i].filled_mask & CHDATA_NOISE_FLOOR) {
...
static int hostapd_ctrl_iface_get_he_phy_channel_width_set  (struct hostapd_data *hapd, const char *cmd, char *reply, size_t reply_len)
{
	...
	if (hwaddr_aton(cmd, sta_mac)) {
		ret = os_snprintf(reply, reply_len, "CTRL: GET_HE_PHY_CHANNEL_WIDTH_SET - Incorrect MAC address passed\n");
		if (os_snprintf_error(reply_len, ret))
			return 0;
		return ret;
	}
	...
	if (!sta) {
		ret = os_snprintf(reply, reply_len, "CTRL: GET_HE_PHY_CHANNEL_WIDTH_SET - STA doesn't exist\n");
		if (os_snprintf_error(reply_len, ret))
			return 0;
		return ret;
	}
	...
	if (!sta->last_assoc_req || !sta->last_assoc_req_len) {
		ret = os_snprintf(reply, reply_len, "CTRL: GET_HE_PHY_CHANNEL_WIDTH_SET - STA doesn't have last assoc req\n");
		if (os_snprintf_error(reply_len, ret))
			return 0;
		return ret;
	}
	...
	if ((NULL == sta->he_capabilities) || (sta->he_capabilities_len_from_sta < 1)) {
		ret = os_snprintf(reply, reply_len, "CTRL: GET_HE_PHY_CHANNEL_WIDTH_SET - STA doesn't have HE elements in assoc req\n");
		if (os_snprintf_error(reply_len, ret))
			return 0;
		return ret;
	}
	...
	ret = os_snprintf(reply + len, reply_len - len, "he_phy_chanwidth_set=20MHz");
	if (os_snprintf_error(reply_len - len, ret))
		return len;
	len += ret;

	if(he_caps->he_phy_capab_info[HE_PHYCAP_CAP0_IDX] & HE_PHY_CAP0_CHANNEL_WIDTH_SET_B0) {
			ret = os_snprintf(reply + len, reply_len - len, ",40MHz");
			if (os_snprintf_error(reply_len - len, ret))
				return len;
			len += ret;
	}
	...
	if(he_caps->he_phy_capab_info[HE_PHYCAP_CAP0_IDX] & HE_PHY_CAP0_CHANNEL_WIDTH_SET_B1) {
			ret = os_snprintf(reply + len, reply_len - len, ",80MHz");
			if (os_snprintf_error(reply_len - len, ret))
				return len;
			len += ret;
	}
	...
	if(he_caps->he_phy_capab_info[HE_PHYCAP_CAP0_IDX] & HE_PHY_CAP0_CHANNEL_WIDTH_SET_B2) {
			ret = os_snprintf(reply + len, reply_len - len, ",160MHz");
			if (os_snprintf_error(reply_len - len, ret))
				return len;
			len += ret;
	}
	...
	if(he_caps->he_phy_capab_info[HE_PHYCAP_CAP0_IDX] & HE_PHY_CAP0_CHANNEL_WIDTH_SET_B3) {
			ret = os_snprintf(reply + len, reply_len - len, ",80+80MHz");
			if (os_snprintf_error(reply_len - len, ret))
				return len;
			len += ret;
	}
...
static int hostapd_ctrl_iface_get_sta_he_caps(struct hostapd_data *hapd, const char *cmd, char *reply, size_t reply_len)
{
	...
	if (hwaddr_aton(cmd, sta_mac)) {
		ret = os_snprintf(reply, reply_len, "CTRL: GET_STA_HE_CAPS - Incorrect MAC address passed\n");
		if (os_snprintf_error(reply_len, ret))
			return 0;
		return ret;
	}
	...
	if (!sta) {
		ret = os_snprintf(reply, reply_len, "CTRL: GET_STA_HE_CAPS - STA doesn't exist\n");
		if (os_snprintf_error(reply_len, ret))
			return 0;
		return ret;
	}
	...
	if (!sta->last_assoc_req || !sta->last_assoc_req_len) {
		ret = os_snprintf(reply, reply_len, "CTRL: GET_STA_HE_CAPS - STA doesn't have last assoc req\n");
		if (os_snprintf_error(reply_len, ret))
			return 0;
		return ret;
	}
	...
	if ((NULL == sta->he_capabilities) || (sta->he_capabilities_len_from_sta < 1)) {
		ret = os_snprintf(reply, reply_len, "CTRL: GET_STA_HE_CAPS - STA doesn't have HE elements in assoc req\n");
		if (os_snprintf_error(reply_len, ret))
			return 0;
		return ret;
	}
	...
	ret = os_snprintf(reply + len, reply_len - len, "he_phy_chanwidth_set=");
	if (os_snprintf_error(reply_len - len, ret))
		return len;
	len += ret;

	if(he_caps->he_phy_capab_info[HE_PHYCAP_CAP0_IDX] & HE_PHY_CAP0_CHANNEL_WIDTH_SET_B0) {
			ret = os_snprintf(reply + len, reply_len - len, "B0");
			if (os_snprintf_error(reply_len - len, ret))
				return len;
			len += ret;
	}
	...
	if(he_caps->he_phy_capab_info[HE_PHYCAP_CAP0_IDX] & HE_PHY_CAP0_CHANNEL_WIDTH_SET_B1) {
			ret = os_snprintf(reply + len, reply_len - len, ",B1");
			if (os_snprintf_error(reply_len - len, ret))
				return len;
			len += ret;
	}
	...
	if(he_caps->he_phy_capab_info[HE_PHYCAP_CAP0_IDX] & HE_PHY_CAP0_CHANNEL_WIDTH_SET_B2) {
			ret = os_snprintf(reply + len, reply_len - len, ",B2");
			if (os_snprintf_error(reply_len - len, ret))
				return len;
			len += ret;
	}
	...
	if(he_caps->he_phy_capab_info[HE_PHYCAP_CAP0_IDX] & HE_PHY_CAP0_CHANNEL_WIDTH_SET_B3) {
			ret = os_snprintf(reply + len, reply_len - len, ",B3");
			if (os_snprintf_error(reply_len - len, ret))
				return len;
			len += ret;
	}
	...
	if(he_caps->he_phy_capab_info[HE_PHYCAP_CAP0_IDX] & HE_PHY_CAP0_CHANNEL_WIDTH_SET_B4) {
			ret = os_snprintf(reply + len, reply_len - len, ",B4");
			if (os_snprintf_error(reply_len - len, ret))
				return len;
			len += ret;
	}
	...
	if(he_caps->he_phy_capab_info[HE_PHYCAP_CAP0_IDX] & HE_PHY_CAP0_CHANNEL_WIDTH_SET_B5) {
			ret = os_snprintf(reply + len, reply_len - len, ",B5");
			if (os_snprintf_error(reply_len - len, ret))
				return len;
			len += ret;
	}
	...
	if(he_caps->he_phy_capab_info[HE_PHYCAP_CAP0_IDX] & HE_PHY_CAP0_CHANNEL_WIDTH_SET_B6) {
			ret = os_snprintf(reply + len, reply_len - len, ",B6(reserved)");
			if (os_snprintf_error(reply_len - len, ret))
				return len;
			len += ret;
	}
	...
	ret = os_snprintf(reply + len, reply_len - len, "he_phy_he_er_su_ppdu_with_4x_he_ltf_and_08_us_gi=%d\n",
		(he_caps->he_phy_capab_info[HE_PHYCAP_CAP7_IDX] & HE_PHY_CAP7_SU_PPDU_AND_HE_MU_WITH_4X_HE_LTF_0_8US_GI)>>2);
	if (os_snprintf_error(reply_len - len, ret))
		return len;
	len += ret;

	ret = os_snprintf(reply + len, reply_len - len, "he_phy_20_mhz_in_40_mhz_he_ppdu_in_24_ghz_band=%d\n",
		(he_caps->he_phy_capab_info[HE_PHYCAP_CAP8_IDX] & HE_PHY_CAP8_20MHZ_IN_40MHZ_HE_PPDU_2_4_GHZ_BAND)>>1);
	if (os_snprintf_error(reply_len - len, ret))
		return len;
	len += ret;

	ret = os_snprintf(reply + len, reply_len - len, "he_phy_20_mhz_in_160_slash_80_plus_80_mhz_he_ppdu=%d\n",
					      ...
					      socklen_t fromlen)
{
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		if(!bss)
			...
			buf_start = buf + sizeof("ACCEPT_ACL ") - 1 + os_strlen(bss->conf->iface) + sizeof(" ") - 1;
		...
		if(!bss)
			...
			buf_start = buf + sizeof("DENY_ACL ") - 1 + os_strlen(bss->conf->iface) + sizeof(" ") - 1;

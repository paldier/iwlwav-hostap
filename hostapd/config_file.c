			       ...
			       const char *buf, char *pos, int line)
{
    ...
    */
    ...
    for (i = 0; i < ACS_NUM_GRP_PRIORITIES; i++) {
      if ((conf->acs_grp_priorities_throughput[i] >= ACS_NUM_GRP_PRIORITIES) ||
          (conf->acs_grp_priorities_throughput[i] < 0)) {
    ...
    for (i = 0; i < ACS_NUM_GRP_PRIORITIES; i++) {
      if ((conf->acs_grp_priorities_reach[i] >= ACS_NUM_GRP_PRIORITIES) ||
          (conf->acs_grp_priorities_reach[i] < 0)) {
  ...
  else if (os_strcmp(buf, "acs_bw_threshold") == 0) {
    int i = 0;
    if (hostapd_parse_intlist(&conf->acs_bw_threshold, pos)) {
      wpa_printf(MSG_ERROR, "Line %d: invalid ACS BW threshold list", line);
      return 1;
    }
    for (i = 0; i < ACS_BW_THRESH_NUM; i++) {
      if ((conf->acs_bw_threshold[i] > 100) ||
          (conf->acs_bw_threshold[i] < 0)) {
        wpa_printf(MSG_ERROR, "Line %d: invalid ACS BW threshold list entry %i", line, i);
      }
    }
  }
#endif /* CONFIG_ACS */
		...
		if ((val != MGMT_FRAMES_RATE_DISABLED) &&
			((val < MGMT_FRAMES_RATE_ENABLED_MIN) ||
			 (val > MGMT_FRAMES_RATE_ENABLED_MAX)))
		{
	  ...
	  conf->ht_mcs_set[3] = host_to_le32(atoi(pos));
		...
		conf->ieee80211n_acax_compat = atoi(pos);
#ifdef CONFIG_IEEE80211AC
...
#define PARSE_TEST_PROBABILITY(_val)				\
	...
	PARSE_TEST_PROBABILITY(corrupt_gtk_rekey_mic_probability)
		...
		if (bss->enable_bss_load_ie < 0 || bss->enable_bss_load_ie > 1) {
		...
		bss->s11nProtection = atoi(pos);

	} else if (os_strcmp(buf, "sDisableSoftblock") == 0) {
		bss->sDisableSoftblock = atoi(pos);

	} else if (os_strcmp(buf, "sFourAddrMode") == 0) {
		bss->sFourAddrMode = atoi(pos);
		...
		if(hostapd_parse_intlist(&conf->sRTSmode, pos))
			return 1;
		...
		if(hostapd_parse_intlist(&conf->sInterferDetThresh, pos))
			return 1;

	} else if (os_strcmp(buf, "sCcaAdapt") == 0) {
		if(hostapd_parse_intlist(&conf->sCcaAdapt, pos))
			return 1;
